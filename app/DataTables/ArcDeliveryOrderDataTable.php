...
class ArcDeliveryOrderDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->addColumn('action', 'archived.delivery_orders.datatables_actions');
    }

    /**
     * Get query source of dataTable.
     *
     * @param \App\Models\ArcDeliveryOrder $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(ArcDeliveryOrder $model)
    {
        ...
        {
            $join->on('arc_deliveryorders.id', '=', 'claims.deliveryorder_id');
        })
        ->select('arc_deliveryorders.id','arc_deliveryorders.dono','arc_deliveryorders.date','arc_deliveryorders.driver_id','arc_deliveryorders.lorry_id','arc_deliveryorders.vendor_id','arc_deliveryorders.source_id','arc_deliveryorders.remark',
        'arc_deliveryorders.destinate_id','arc_deliveryorders.item_id','arc_deliveryorders.weight','arc_deliveryorders.shipweight','arc_deliveryorders.fees','arc_deliveryorders.tol','arc_deliveryorders.billingrate','arc_deliveryorders.commissionrate','arc_deliveryorders.status','arc_deliveryorders.archived_at',
        DB::raw('CONCAT(arc_deliveryorders.id,":",arc_deliveryorders.billingrate) as billingrate_data'),
        DB::raw('CONCAT(arc_deliveryorders.id,":",arc_deliveryorders.commissionrate) as commissionrate_data'),
        DB::raw('CONCAT(arc_deliveryorders.id,":",COUNT(claims.id)) as claims'))
        ->groupby('arc_deliveryorders.id','arc_deliveryorders.dono','arc_deliveryorders.date','arc_deliveryorders.driver_id','arc_deliveryorders.lorry_id','arc_deliveryorders.vendor_id','arc_deliveryorders.source_id','arc_deliveryorders.remark',
        'arc_deliveryorders.destinate_id','arc_deliveryorders.item_id','arc_deliveryorders.weight','arc_deliveryorders.shipweight','arc_deliveryorders.fees','arc_deliveryorders.tol','arc_deliveryorders.billingrate','arc_deliveryorders.commissionrate','arc_deliveryorders.status','arc_deliveryorders.archived_at');
    }
        ...
        return $this->builder()
            ...
            ->parameters([
                ...
                'columnDefs' => [
                    ...
                    [
                        'targets' => 12,
                        'render' => 'function(data, type){return parseFloat(data.split(":")[1]).toFixed(2).split(".")[0].replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") + (parseFloat(data.split(":")[1]).toFixed(2).split(".")[1] ? "." + parseFloat(data.split(":")[1]).toFixed(2).split(".")[1] : "");}'
                        ,'className' => 'dt-body-right'
                    ],
                    [
                        'targets' => 13,
                ...
                'initComplete' => 'function(){
                    ...
                    .every(function (index) {
                        ...
                        if(columns[index].searchable){
                            ...
                            }else if(columns[index].title == \'Archived Date\'){
                                var input = \'<input type="text" id="\'+index+\'Date" onclick="searchDateColumn(this);" placeholder="Search ">\';
                            }
                            else{
    ...
    protected function getColumns()
    {
        return [
            ...
            'data' => 'remark',
            'name' => 'arc_deliveryorders.remark']),
            'archived_at'=> new \Yajra\DataTables\Html\Column(['title' => 'Archived Date',
            'data' => 'archived_at',
            'name' => 'arc_deliveryorders.archived_at']),
