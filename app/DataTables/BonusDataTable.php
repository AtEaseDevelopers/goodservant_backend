...
class BonusDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->addColumn('action', 'bonuses.datatables_actions');
    }
    ...
    {
        return $model->newQuery()->with('vendor:id,code')->with('source:id,code')->with('destinate:id,code')->select('bonus.*');
    }
        ...
        return $this->builder()
            ...
            ->parameters([
                ...
                'buttons'   => [
                    ...
                    ['extend' => 'excelHtml5','text'=>'<i class="fa fa-file-excel-o"></i> Excel','exportOptions'=> ['columns'=>':visible:not(:last-child)'], 'className' => 'btn btn-default btn-sm no-corner','title'=>null,'filename'=>'Bonus'.date('dmYHis')],
                    ['extend' => 'pdfHtml5', 'orientation' => 'landscape', 'pageSize' => 'LEGAL','text'=>'<i class="fa fa-file-pdf-o"></i> PDF','exportOptions'=> ['columns'=>':visible:not(:last-child)'], 'className' => 'btn btn-default btn-sm no-corner','title'=>null,'filename'=>'Bonus'.date('dmYHis')],
                ...
                'initComplete' => 'function(){
                    ...
                    .every(function (index) {
                        ...
                        if(columns[index].searchable){
                            ...
                            }else if(columns[index].title == \'Bonus Start\'){
                                var input = \'<input type="text" id="\'+index+\'Date" onclick="searchDateColumn(this);" placeholder="Search ">\';
                            }else if(columns[index].title == \'Bonus End\'){
                                var input = \'<input type="text" id="\'+index+\'Date" onclick="searchDateColumn(this);" placeholder="Search ">\';
                            }else{
    ...
    protected function getColumns()
    {
        return [
            ...
            // 'name' => 'destinate.code']),
            'target',
            'bonusstart'=> new \Yajra\DataTables\Html\Column(['title' => 'Bonus Start',
            'data' => 'bonusstart',
            'name' => 'bonusstart']),
            'bonusend'=> new \Yajra\DataTables\Html\Column(['title' => 'Bonus End',
            'data' => 'bonusend',
            'name' => 'bonusend']),
            'amount',
            'status',
    ...
    {
        return 'bonuses_datatable_' . time();
    }
}
